...
#[test]
fn unsupported_notation_produces_a_diagnostic() {
    let env = parse_env("sort s; term z: s; prefix z: $0$ prec max;").unwrap();

    assert_eq!(env.diagnostics.len(), 1);
    assert!(env.diagnostics[0].message.contains("unsupported notation"));
}
...
fn parses_copied_third_party_fixture_inventory() {
    let cases = [
        ("aufbau_pass_def", AUFBAU_PASS_DEF, 1, 2, 2),
        (
        ...
        ("mm0_hello", MM0_HELLO, 3, 30, 0),
        ("mm0_peano", MM0_PEANO, 3, 116, 81),
    ];

    for (name, input, sorts, terms, theorems) in cases {
...
fn parses_aufbau_relation_congruence_and_ignored_rewrites() {
    ...
    assert!(sb_p.conclusion.unsupported_reason.is_some());
}
